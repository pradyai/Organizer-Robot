...
class Stack(ManipulationEnv):
    ...
    def staged_rewards(self):
        ...
        # ball_pos = self.sim.data.body_xpos[self.ball_body_id] if hasattr(self, 'ball_body_id') else None
        cylinder_pos = self.sim.data.body_xpos[self.cylinder_body_id] if hasattr(self, 'cylinder_body_id') else None
        dist = min(
            [
        ...
        # ball_height = ball_pos[2] if ball_pos is not None else None
        cylinder_height = cylinder_pos[2] if cylinder_pos is not None else None
    ...
    def _load_model(self):
        ...
        objects = [self.cubeA, self.cubeB, self.ball, self.cylinder]
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            def ball_pos(obs_cache):
                return np.array(self.sim.data.body_xpos[self.ball_body_id]) if hasattr(self, 'ball_body_id') else np.zeros(3)

            @sensor(modality=modality)
            def cylinder_pos(obs_cache):
                return np.array(self.sim.data.body_xpos[self.cylinder_body_id]) if hasattr(self, 'cylinder_body_id') else np.zeros(3)

            @sensor(modality=modality)
            def ball_quat(obs_cache):
                return convert_quat(np.array(self.sim.data.body_xquat[self.ball_body_id]), to="xyzw") if hasattr(self, 'ball_body_id') else np.zeros(4) 
            @sensor(modality=modality)
            def cylinder_quat(obs_cache):
                return convert_quat(np.array(self.sim.data.body_xquat[self.cylinder_body_id]), to="xyzw") if hasattr(self, 'cylinder_body_id') else np.zeros(4)
            ...
            sensors = [cubeA_pos, cubeA_quat, cubeB_pos, cubeB_quat, cubeA_to_cubeB, ball_pos, ball_quat, cylinder_pos, cylinder_quat]
            sensors += [
