...
class PolicyServer(services_pb2_grpc.AsyncInferenceServicer):
    ...
    def request_smolvla_action(self, timed_obs, host="127.0.0.1", port=6006):
        """
        Send a TimedObservation to the SmolVLA socket server and get back action tensor.
        Converts all numpy arrays in observation to torch tensors with correct shapes.
        """
        ...
        for key, val in obs_dict.items():
            ...
            if "images" in key:
                ...
                val = val.unsqueeze(0)  # add batch dimension: (1,C,H,W)
            elif "state" in key:
                if val.ndim == 1:
                    val = val.unsqueeze(0)  # (1,state_dim)
            obs_dict[key] = val
