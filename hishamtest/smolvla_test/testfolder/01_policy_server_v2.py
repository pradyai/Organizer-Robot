...
class PolicyServer(services_pb2_grpc.AsyncInferenceServicer):
    ...
    def request_smolvla_action(batch, host="127.0.0.1", port=6006):
        """ 
        Send an observation batch to the SmolVLA socket server and get back action tensor.
        batch: dict with keys 'observation.images.top', 'observation.state', 'task'
        """
        # Convert torch tensors to numpy for pickling
        batch_to_send = batch.copy()
        for k in ["observation.images.top", "observation.state"]:
            batch_to_send[k] = batch[k].cpu().numpy()

        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            sock.connect((host, port))
            msg = pickle.dumps(batch_to_send)
            ...
            while len(data) < msg_len:
                ...
                if not packet:
                    raise RuntimeError("SmolVLA server closed connection mid-response")
                data += packet

            # Deserialize
            actions = pickle.loads(data)
            return torch.tensor(actions)  # ensure torch tensor
    ...
    def _get_action_chunk(self, observation: dict[str, torch.Tensor], task: str = "stack the blocks") -> torch.Tensor:
        """Get an action chunk from the SmolVLA socket server including all images, state, and task"""

        # Construct the batch for SmolVLA server
        batch = {
            "observation.images.top": observation["images.top"].unsqueeze(0),    # add batch dim
            "observation.images.side": observation["images.side"].unsqueeze(0),  # add batch dim
            "observation.state": observation["state"].unsqueeze(0),
            "task": [task],  # send task as list
        }

        # Send to SmolVLA server
        chunk = self.request_smolvla_action(batch, host="127.0.0.1", port=6006)
