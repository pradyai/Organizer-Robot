...
#here define a send_action thread to send the data to the robot asynchronously ----
...
try:
    ...
    while True:
        dummy_obs_dict = {
            "observation.images.top": torch.rand(batch_size, *img_shape), #here, put the actual camera observation, use robosuite api
        ...
        print(actions.shape) # torch.Size([1, 50, 32]), send only one setpoint vector at a time, reduce to 6 dim 
        send_actions(actions) # send the actions to the robot IIMPORTANT --------------------------------------------------------------------------------. 
        sock.close()
